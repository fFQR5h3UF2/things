...
type SubmissionsDownloader struct {
	config *model.Config
	client *http.Client
	ctx    context.Context
	logger *slog.Logger
...
func NewSubmissionsDownloader(config *model.Config, ctx context.Context, logger *slog.Logger) (*SubmissionsDownloader, error) {
	if config.Session == "" {
		return nil, fmt.Errorf("missing cookie in the config")
	}
	downloader := &SubmissionsDownloader{
		config: config,
		client: &http.Client{},
		ctx:    ctx,
		logger: logger,
...
func (d *SubmissionsDownloader) GetSubmissions(offset uint64, limit uint64) ([]*model.Submission, error) {
...
func (d *SubmissionsDownloader) getSubmissions(offset uint64, limit uint64) (*model.SubmissonsResponse, error) {
