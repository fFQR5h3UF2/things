...
// Submission json : {"id":691682236,"status_display":"Accepted","lang":"golang","question_id":35,"title_slug":"search-insert-position","code":"func searchInsert(numbers []int, target int) int {\n\t// checking edge cases\n\tlength := len(numbers)\n\tif numbers[0] == target {\n\t\treturn 0\n\t} else if numbers[length-1] == target {\n\t\treturn length - 1\n\t}\n\tleft, right, index, was_bigger := 0, length-1, 0, false\n\tfor right >= left {\n\t\t// overflow protection\n\t\tindex = left + (right-left)/2\n\t\tnumber := numbers[index]\n\t\t//fmt.Println(\"index\", index, \"left\", left, \"right\", right)\n\t\tswitch {\n\t\tcase number == target:\n\t\t\t// found the target\n\t\t\treturn index\n\t\tcase number > target:\n\t\t\t// the number is bigger -> the target is to the left -> discard right\n\t\t\tright = index - 1\n\t\t\twas_bigger = true\n\t\tcase number < target:\n\t\t\t// the number is smaller -> the target is to the right -> discard left\n\t\t\tleft = index + 1\n\t\t\twas_bigger = false\n\t\t}\n\t}\n\t// the target is not in the array\n\n\t// the last number was bigger -> target should be to the left\n\tif was_bigger {\n\t\treturn index\n\t}\n\t// the last number was smaller -> target should be to the right\n\treturn index + 1\n}\n","title":"Search Insert Position","url":"/submissions/detail/691682236/","lang_name":"Go","time":"1 year, 9 months","timestamp":1651506546,"status":10,"runtime":"4 ms","is_pending":"Not Pending","memory":"3 MB","compare_result":"1111111111111111111111111111111111111111111111111111111111111111","has_notes":false,"flag_type":1}
package submissions
