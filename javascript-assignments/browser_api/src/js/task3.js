(() => {
    ...
    function getTask() {
        ...
        content.setAttribute('class', 'm-3 p-3 messages rounded-30 border border-secondary bg-secondary d-flex flex-column justify-content-center');
        ...
        buttonGeolocaton.setAttribute('class', 'my-1 mx-1 btn btn-dark rounded-30');
        ...
        output.setAttribute('class', 'd-flex flex-column p-3 my-3 bg-dark outline-secondary');
            ...
            if (event.data !== 'location') {
                ...
                if ("geolocation" in navigator) {
                    navigator.geolocation.getCurrentPosition((position) => {
                        ...
                        sendMessage('https://www.openstreetmap.org/#map=4/' + coords.latitude + '/' + coords.longitude, 2);
                    });
                }
            }
        };
    ...
    function sendMessage(text, type) {
        document.getElementById('task3-output').appendChild(getMessage(text, type));
    }
    ...
    function getMessage(text, type = 0) {
        ...
        messageNode.setAttribute('class', `d-flex flex-row flex-wrap ${justify} my-1 `);
        const message = document.createElement(`${tag}`);
        message.setAttribute('class', 'p-1 message bg-secondary text-light rounded-3');
        if (tag === 'a') {
